...
- `--units <u>`: 'output' (default) or 'model', what the flat domain's lengths below are given in
- `--width <w>` / `--height <h>`: The flat domain's rectangle (cylinder circumference and height; default: 2 x 1 model units)
- `--diameter <d>`: The cylinder's diameter, in place of `--width`
