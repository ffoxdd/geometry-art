...
struct TorusDiagramTraits {
    ...
    static std::unique_ptr<Torus> scattered(size_t count) { return globe::testing::scattered_torus(count, WIDTH, HEIGHT); }
...
struct TorusStateTraits {
    static voronoi::DiagramState scattered_state(size_t count) {
        ...
        return lagrangian.diagram_state(*globe::testing::scattered_torus(count, WIDTH, HEIGHT));
    }
