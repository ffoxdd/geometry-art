...
SphereState CapacityConstrainedLagrangian<FieldType>::sphere_state(const Sphere& sphere) const {
    ...
    std::map<EdgeKey, Moments> arc_moments_by_edge;
    SphereState state;
    state.cells.reserve(count);
    state.edges.resize(count);

    for (size_t k = 0; k < count; ++k) {
        std::vector<CellEdgeInfo> cell_edges = sphere.cell_edges(k);
        std::vector<Arc> arcs;
        std::vector<Moments> arc_moments;
        arcs.reserve(cell_edges.size());
        arc_moments.reserve(cell_edges.size());
        state.edges[k].reserve(cell_edges.size());

        for (const CellEdgeInfo& edge : cell_edges) {
            auto [iterator, inserted] = arc_moments_by_edge.try_emplace(edge_key(k, edge.neighbor_index), moment_degree);
            if (inserted) {
                iterator->second = edge.arc.moments(moment_degree);
            }
